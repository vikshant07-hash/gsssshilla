    ...
    await axios.post(
      "https://api.brevo.com/v3/smtp/email",
      {
        sender: {
          ...
          email: process.env.BREVO_SENDER_EMAIL  // .env mein set karo
        },
        to: [{ email: to }],
        subject: title,
...
function generateOTP() {
  ...
  let otp = "";
  for (let i = 0; i < 2; i++) otp += letters[Math.floor(Math.random() * letters.length)];
  for (let i = 0; i < 4; i++) otp += numbers[Math.floor(Math.random() * numbers.length)];
  return otp;
}
...
function getOTPTemplate(otp, title) {
  ...
  <div style="font-family:Arial;background:#0b1220;padding:30px;color:#fff">
    <div style="max-width:500px;margin:auto;background:#111827;padding:20px;border-radius:12px;text-align:center">
      ...
      <h3>${title}</h3>
      <div style="margin:20px auto;padding:15px;background:#0f172a;border:2px dashed #ce0c9d;font-size:28px;letter-spacing:5px;font-weight:bold;">
        ${otp}
      </div>
...
router.post("/send-otp", (req, res) => {
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      db.query("UPDATE admins SET otp=?, otp_expiry=? WHERE id=?", [otp, expiry, user.id]);
 
      const sent = await sendEmail(user.email, otp, "ADMIN LOGIN OTP");
